...
const CreatePropertyCategory: React.FC = () => {
  ...
  const formatTreeData = (node: any): DefaultOptionType => ({
    value: node.id,  
    title: node.title, 
    children: node.children?.map((child: any) => formatTreeData(child)) || [], // Gọi đệ quy để xử lý cây con
  });
  ...
  const onFinishForm = async (data: any) => {
    try {
      ...
      if (response.status >= 200 && response.status < 300 ) {
        ...
        console.error('Error occurred while creating category: ', response.data.message);
                    ...
                    <Badge.Ribbon text={<Link to="/admin/property-categories">Back</Link>} color="purple" className="custom-ribbon">
                      ...
                      >
                        <Row gutter={16}>
                          ...
                          {/* <Col sm={24} md={24} lg={13} xl={13} xxl={13}>
                            <Form.Item label="Post position:" name='position'>
                              <InputNumber 
                                type="number"
                                id="position" 
                                min={0} 
                                className="custom-number-input position-input w-100"
