...
public interface PostServiceRepository extends JpaRepository<PostService, Long>, PostServiceRepositoryCustom {
    ...
    @Query("SELECT ps.availableUnits FROM post_service ps WHERE ps.postServiceId IN :ids")
    List<Integer> getAvailableUnitsByPostServiceId(@Param("ids") Iterable<Long> ids);
